...
def make_neuron_layer(input_size, output_size):
    ...
    return NeuronLayer(torch.tensor(w, requires_grad=True, dtype=float), torch.tensor(b, requires_grad=True, dtype=float))
...
def train_until_pass(model, pass_rate, data):
    try:
        ...
        output_all_case(XOR_model1, training_data)

output_all_case(XOR_model1, training_data)

fixed_training(XOR_model1, 1000, training_data)
